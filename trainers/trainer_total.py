...
list_sr_model_type_b = ["ESRT", "HAN", "IMDN", "BSRN", "RFDN", "PAN", "LAPAR","CNCAN"]
...
def trainer_(**kargs):
    ...
    trainer_(# <patch를 통해 학습 & RGB 이미지를 생성하는 모델>#
            ...
            ,path_check_point               = path_check_point
             ...
             # SSSR & SS mIoU -> 0.xxxx 형태로 입력 , SR PSNR -> xx.xxxx 형태로 입력
            ,prev_best                      = prev_best
            
            #(선택) 학습 초반 이후 Test에서 NIQE 측정여부 (default: False)
            ,will_niqe_calcurated           = (bool) 
            ...
            ,PATH_ALTER_HR_IMAGE            = "some_whole_path"
            ...
            ,PATH_OUT_LOG                   = PATH_OUT_LOG
             ...
             # Patch 생성 관련 강제 Margin 추가여부 (bool, lr 이미지 관련 값은 scale factor에 비례한 값 자동 입력됨)
            ,is_force_fix                   = # (bool, default = False)
            ,force_fix_size_hr              = # (tuple with int) (w, h) margin 포함 크기
            ...
            ,HP_CJ_HUE                      = HP_CJ_HUE
            ,HP_AUGM_RANDOM_SCALER          = [1.0, 1.5, 2.0]           # -> [1.0] 입력시, random scale 적용 안함
            ...
            ,HP_TS_NORM_STD                 = HP_TS_NORM_STD_DSRL
             ...
             # gradient clipping (default = False)
            ,is_gradient_clipped            = (bool)
    ...
    # 데이터 정보 (생략 가능)
    try:
        HP_ORIGIN_IMG_W = kargs['HP_ORIGIN_IMG_W']  # 원본 이미지 W 값
        HP_ORIGIN_IMG_H = kargs['HP_ORIGIN_IMG_H']  # 원본 이미지 H 값
        HP_CHANNEL_RGB  = kargs['HP_CHANNEL_RGB']   # RGB 이미지 채널 값
        HP_CHANNEL_GRAY = kargs['HP_CHANNEL_GRAY']  # Gray 이미지 채널 값
    except:
        HP_ORIGIN_IMG_W = "No Info"
        HP_ORIGIN_IMG_H = "No Info"
    ...
    for i_key in list_mode:
        update_dict_v2("epoch", "loss_(" + i_key + "),mIoU_(" +  i_key + ")," + HP_DATASET_CLASSES
    ...
    for i_epoch in range(HP_EPOCH):
        ...
        for i_mode in list_mode:
            ...
            for i_key in list_mode:
                update_dict_v2(i_key + "_"+ str(i_epoch + 1), "batch_num,file_name,loss_batch,miou_image," + HP_DATASET_CLASSES
                ...
                update_dict_v2(i_key + "_"+ str(i_epoch + 1), "batch_num,file_name," + _str + HP_DATASET_CLASSES
