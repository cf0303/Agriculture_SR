...
class Custom_Dataset_V6(data.Dataset):
    ...
    dataset_train = Custom_Dataset_V6(# Return: dict key order -> 'file_name'
                                     ...
                                     ,opt_augm_lite                 = False
                                     ,opt_augm_lite_flip_hori       = False
                                     ,opt_augm_lite_flip_vert       = False
    ...
    def __init__(self, **kargs):
        ...
        if self.is_train:
            try:
                ...
                self.opt_augm_lite_flip_hori = kargs['opt_augm_lite_flip_hori']     # (bool) random 수평 flip 여부
                self.opt_augm_lite_flip_vert = kargs['opt_augm_lite_flip_vert']     # (bool) random 수직 flip 여부
            except:                                                                 # 아래의 Flip Crop Rotate & color jitter와 동시적용 불가
                self.opt_augm_lite           = False                                # Label 이미지 사용 시엔 적용 불가능
                self.opt_augm_lite_flip_hori = False
                self.opt_augm_lite_flip_vert = False
            
            if self.opt_augm_lite == False:
                ...
                print(self.name_func, "Augmentation LITE mode 활성화. LITE mode의 경우, label data & color jitter 옵션 사용 불가")
                print(self.name_func, "Flip Horizontal:", self.opt_augm_lite_flip_hori)
                print(self.name_func, "Flip Vertical:", self.opt_augm_lite_flip_vert)
                self.opt_augm_crop_init_range   = None
                self.opt_augm_rotate_max_degree = None
                self.opt_augm_prob_flip         = None
                self.opt_augm_prob_crop         = None
                self.opt_augm_prob_rotate       = None
                
                self.opt_augm_cj_brigntess      = [1,1]
                self.opt_augm_cj_contrast       = [1,1]
                self.opt_augm_cj_saturation     = [1,1]
                self.opt_augm_cj_hue            = [0,0]
        ...
        self.in_path_dataset          = kargs['in_path_dataset']                    # (str)  ┐
        #                                                                                                  │
        ...
        if self.is_return_image_lr:
            self.in_path_dlc = kargs['in_path_dlc']                                 # (str) ┐
            #                                                                                                        │
        ...
        # pil_2_patch options
        try:
            self.is_force_fix = kargs['is_force_fix']                           # (bool) (train, val) 최종 이미지의 강제 margin 추가여부 - patch 생성겸용
        except:
        ...
        #del self.dict_pil_raw_img_hr
        #del self.dict_pil_raw_lab_hr
        #del self.dict_pil_raw_img_lr
    ...
    def __getitem__(self, idx):
        ...
        if self.in_path_alter_hr_image is None:
            # 원본 폴더 내용물 사용
            path_img_hr = self.in_path_dataset + self.in_category + self.in_name_folder_image + '/' + self.list_file_img[idx]
        else:
            # 대체 폴더 내용물 사용
            path_img_hr = self.in_path_alter_hr_image + self.list_file_img[idx]
        ...
        if self.is_return_label:
            #<<<
            path_lab_hr = self.in_path_dataset + self.in_category + self.in_name_folder_label + '/' + self.list_file_lab[idx]
            pil_lab_hr_raw = Image.open(path_lab_hr)                            # (pil) HR label
            #>>>    
        ...
        if self.is_return_image_lr:
            #<<<
            path_img_lr = self.in_path_dlc + self.in_name_folder_image + '/' + self.list_file_img[idx]
            pil_img_lr_raw = Image.open(path_img_lr)                            # (pil) LR image
            #>>>
        ...
        if self.is_train and self.opt_augm_lite:
            out_pil_img_hr, out_pil_img_lr, str_augm_option = pil_augm_lite(out_pil_img_hr
                                                                           ,out_pil_img_lr
                                                                           ,self.opt_augm_lite_flip_hori
                                                                           ,self.opt_augm_lite_flip_vert
                                                                           ,get_info=True
                                                                           )
